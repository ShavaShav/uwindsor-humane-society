...
// 2017 Winter 03-60-334-30 course for use within their
...
ZZEOF;
  ...
  public function admin_destroy_all_structures()
  {
    ...
    // NOTE: There is no need to use an SQL prepared statement call
    //       here since the SQL code is 100% from this site and safe.
    $sql = "DROP TABLE IF EXISTS users_test";
    return $this->db_handle()->exec($sql);
  }
  ...
  //
  // Sites explaining why:
  //   https://wblinks.com/notes/storing-passwords-the-wrong-better-and-even-better-way/
  //   https://nakedsecurity.sophos.com/2013/11/20/serious-security-how-to-store-your-users_test-passwords-safely/
  //
  ...
  public function insert($user, $pass)
  {
    ...
    $sql = 'INSERT INTO users_test VALUES (:user, :pass)';
  ...
  public function erase($user)
  {
    ...
    $sql = 'DELETE FROM users_test WHERE user = :user';
    $stmt = $this->db_handle()->prepare($sql);
    $stmt->execute($entry);
  }
  ...
  public function check_user_pass($user, $pass)
  {
    ...
    try
    {
      $sql = 'SELECT pass FROM users_test WHERE user = :user';
      $stmt = $this->db_handle()->prepare($sql);
      $stmt->execute($entry);
  ...
  public function lookup($user)
  {
    ...
    try
    {
      $sql = 'SELECT * FROM users_test WHERE user = :user';
      $stmt = $this->db_handle()->prepare($sql);
      $stmt->execute($entry);
      $result = $stmt->fetchAll();
      if (count($result) != 1)
  ...
  // lookup_all()
  //
  // Look up all users_test in the users_test table. This function permits
